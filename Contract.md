...
###Методы объекта
 Метод | Описание
 --------------------------------------------------------- | ----------------------------------------------
 [GetRejectionComment](GetRejectionComment (Contract)) | Возвращает комментарий к отказу в подписании
 [Accept](Accept (Contract)) | Формирует подпись получателя документа
 [Reject](Reject (Contract)) | Формирует отказ в подписании документа
