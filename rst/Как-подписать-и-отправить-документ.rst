...
| которыми установлены партнерские отношения (см. раздел `"Как
  установить
  партнерские отношения" <#1S_KAOwerview>`__).
...
| метод `CreateSendTaskFromFile <CreateSendTaskFromFile>`__ объекта
| `Organization <Организация>`__. В качестве параметров при вызове
...
| объект `SendTask <SendTask>`__, устанавливая свойства которого
...
| система, как правило, не умеет формировать документы требуемого
  формата.
| В компоненте реализован механизм, позволяющий переложить "заботу" о
| формировании xml-файла на внешнюю компоненту. От учетной системы, в
  этом
